...
describe('VectorTileSource', () => {
    ...
    test('abortTile sets tile state to expired, aborts controller, and sends worker message', async () => {
        const source = createSource({
            tiles: ['http://example.com/{z}/{x}/{y}.pbf']
        });
    ...
    test('abortTile sets tile state to expired and aborts controller even when actor is missing', async () => {
        const source = createSource({
            tiles: ['http://example.com/{z}/{x}/{y}.pbf']
        });
